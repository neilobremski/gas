...
**Request logging** can be enabled by running `Bridge.enableLogging()` from the Apps Script editor. When enabled, every successful request is logged to a Google Sheet named `GAS Log YYYY-MM-DD` (one sheet per UTC day, created automatically). Each row records the timestamp, action name, request parameter keys (not values, to avoid logging sensitive data), and result status. The spreadsheet ID is cached in Script Properties to avoid a Drive search on every request. Logging is wrapped in try/catch so it never breaks the actual request. Run `Bridge.disableLogging()` to turn it off.
