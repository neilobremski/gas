...
**Request logging** can be enabled by running `enableLogging()` from the Apps Script editor (top-level wrapper; or `Bridge.enableLogging()` from the module). When enabled, every request is logged to a Google Sheet named `GAS Log YYYY-MM-DD` (one sheet per UTC day, created automatically). Each row records the timestamp, action name, request parameter keys (not values, to avoid logging sensitive data), and result status. The spreadsheet ID is cached in Script Properties to avoid a Drive search on every request. Logging is wrapped in try/catch so it never breaks the actual request. Run `disableLogging()` to turn it off.

The [A8S participant](../a8s/) uses the same logging mechanism and sheet naming. If both projects run on the same Google account, A8S searches Drive for an existing `GAS Log YYYY-MM-DD` spreadsheet and appends there. A8S rows use action names like `a8s.send` and include a **Notes** column (when A8S creates the sheet) with Markdown diagnostics.
